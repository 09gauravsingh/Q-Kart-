          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
               ...
               (addresses.all.map((item)=>{
                   return (
                     <Box className="address-item" key={item._id} onClick={(e)=>{setAddresses((currAddress) => ({
                      ...currAddress,
                      selected: item._id,
                    }))}}>
                       <Typography>{item.address}</Typography>
                       <Button onClick={(e)=>{deleteAddress(token,item._id)}}
                       variant="text" startIcon={<Delete/>}>DELETE</Button>
                       </Box>
                   )
                 })
               )}
